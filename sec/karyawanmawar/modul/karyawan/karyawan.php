...
<html lang="en">
    ...
    <body>
                                    ...
                                    <div class="adv-table">
                                        <table cellpadding="0" cellspacing="0" border="0" class="display table table-bordered table-striped" id="hidden-table-info">
                                            ...
                                            <tbody>
                                                <?php
                                                $i = 1;
                                                $q = mysql_query("SELECT a.id, a.nik, a.statuspps, a.idpps, a.idalumni, a.nama, CONCAT(a.tempatlahir,', ', DATE_FORMAT(a.tgllahir, '%d-%m-%Y')) AS ttl, "
                                                ...
                                                while ($hasil = mysql_fetch_array($q)) {
                                                    if ($hasil['statuspps'] == 1) {
                                                        $statuspps = "<a href='#' onclick='toLpps(" . $hasil['id'] . ")'>PPS</a>";
                                                        $idpps = $hasil['idpps'];
                                                    } else {
                                                        $statuspps = "<a href='#' onclick='toPps(" . $hasil['id'] . ")'>LPPS</a>";
                                                        $idpps = $hasil['idalumni'];
                                                    }
                                                    echo "<tr class='gradeA'>"
                                                    . "<td>" . $i++ . "</td>"
                                                    . "<td><a rel='tooltip-top' title='Edit' href='upkar.php?id=" . $hasil['id'] . "'>" . $hasil['nik'] . "</a></td>"
                                                    . "<td>" . $idpps . "</td>"
                                                    . "<td>" . $statuspps . "</td>"
                                                    . "<td>" . $hasil['nama'] . "</td>"
                                                    . "<td>" . $hasil['ttl'] . "</td>"
        ...
        <script type="text/javascript">
            function toPps(nik) {
                var person = prompt("Masukkan Nomor IDPPS", "");
                if (person != null) {
                    window.location.href = "proskaryawan.php?type=topps&id=" + nik + "&idpps=" + person;
                }
            }
                function toLpps(nik) {
                var person = prompt("Masukkan Nomor Alumni", "");
                if (person != null) {
                    window.location.href = "proskaryawan.php?type=tolpps&id=" + nik + "&idpps=" + person;
                }
            }
            function delet(nik) {
