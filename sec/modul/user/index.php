...
<html lang="en">
                                    ...
                                    <div class="form">
                                        <form id="inputinv" class="cmxform form-horizontal tasi-form" method="post" action="saveuser.php?jenis=save">
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="text" class="form-control" name="nik" required id="kode" autofocus="" placeholder="NIK KTP">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="text" class="form-control" name="username" required id="nama" placeholder="Username">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="col-sm-3 control-label">Password</label>
                                                <div class="col-sm-8">
                                                    <input type="password" class="form-control" name="password" onkeyup="confirmps()"  required id="password1" placeholder="Password">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="col-sm-3 control-label">Re-Password</label>
                                                <div class="col-sm-8">
                                                    <input type="password" class="form-control" name="repassword" onkeyup="confirmps()" required id="passwordconfirm1" placeholder="Password">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                    ...
                                    <div class="adv-table">
                                        <table cellpadding="0" cellspacing="0" border="0" class="display table table-bordered table-striped" id="hidden-table-info">
                                            ...
                                            <tbody>
                                                <?php
                                                $i = 1;
                                                $q = mysql_query("SELECT noktp, username, nama, otorisasi, status from user");
                                                while ($hasil = mysql_fetch_array($q)) {
                                                    if ($hasil['status'] == 1) {
                                                        $status = "<a rel='tooltip-top' title='Hapus' href='saveuser.php?jenis=hapus&username=" . $hasil['username'] . "'><font color='blue'>Aktif</font></a>";
                                                    } else {
                                                        $status = "<a rel='tooltip-top' title='Aktifkan' href='saveuser.php?jenis=aktif&username=" . $hasil['username'] . "'><font color='red'>Tidak Aktif</font></a>";
                                                    }
                                                    echo "<tr class='gradeX'>"
                                                    . "<td>" . $i++ . "</td>"
                                                    . "<td><a rel='tooltip-top' title='Edit' href='updateuser.php?id=" . $hasil['username'] . "'>" . $hasil['username'] . "</a></td>"
                                                    . "<td><a rel='tooltip-top' title='Edit' href='updateuser.php?id=" . $hasil['nama'] . "'>" . $hasil['nama'] . "</a></td>"
                                                    . "<td>" . $hasil['otorisasi'] . "</td>"
                                                    . "<td>" . $status . "</td>"
                                                    . "<td><a rel='tooltip-top' title='Edit' href='updateuser.php?id=" . $hasil['username'] . "'>"
                                                    . "<i class='icon-edit'></i></a> ";
                                                    if ($hasil['status'] == 1) {
                                                        echo "<a rel='tooltip-top' title='Hapus' href='saveuser.php?jenis=hapus&username=" . $hasil['username'] . "')'><i class='icon-remove'></i></a> </td>";
                                                    } else {
                                                        echo "<a rel='tooltip-top' title='Aktifkan' href='saveuser.php?jenis=aktif&username=" . $hasil['username'] . "'><i class=' icon-rotate-left '></i></a> </td>";
                                                    }
                                                }
                                                ?>
                                            </tbody>
    ...
    <script>
                                                        function confirmps() {
                                                            var passpertama = $('#password1').val();
                                                            var passkedua = $('#passwordconfirm1').val();
                                                            if ($('#password1').val() != $('#passwordconfirm1').val()) {
                                                                $('#passwordconfirm1').css('background', '#e74c3c');
                                                                $('#password1').css('background', '#e74c3c');


                                                            } else if (passpertama == passkedua) {
                                                                $('#passwordconfirm1').css('background', '#2ecc71');
                                                                $('#password1').css('background', '#2ecc71');
