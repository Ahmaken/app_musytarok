...
import { Home, CalendarDays, ClipboardCheck, Bell, User, Moon, Sun, Clock, Menu, X, LogOut, Settings, Users, FileWarning, MessageSquare, UserCog, BookOpen, QrCode } from 'lucide-react';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Fetch Active Schedule dari DB
    fetch('/api/jadwal/active')
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 flex flex-col">
      ...
      <header className="bg-gradient-to-r from-green-800 to-green-900 dark:from-gray-800 dark:to-gray-900 text-white p-4 shadow-md sticky top-0 z-50 flex justify-between items-center rounded-b-2xl transition-colors duration-300 relative">
        ...
        <div className="flex items-center gap-2 relative z-10">
          ...
          <div className="relative ml-1">
            {activeSchedule && (
              <span className="absolute inset-0 rounded-full border-2 border-red-400 animate-ping sm:hidden z-0"></span>
            )}
            <button 
      ...
      {/* Overlay Sidebar */}
      {showSidebar && (
        <div className="fixed inset-0 bg-black/50 z-[60] backdrop-blur-sm transition-opacity" onClick={() => setShowSidebar(false)}></div>
      )}

      {/* Sidebar Drawer */}
      <aside className={`fixed top-0 right-0 h-full w-72 bg-white dark:bg-gray-900 shadow-2xl z-[70] transform transition-transform duration-300 ease-in-out ${showSidebar ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
          ...
          <ul className="space-y-1 px-3 mb-6">
            ...
            <li>
              <Link href="/dashboard/kelas" onClick={() => setShowSidebar(false)} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${pathname === '/dashboard/kelas' ? 'bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 font-bold' : 'hover:bg-teal-50 dark:hover:bg-teal-900/20 text-teal-600 dark:text-teal-400 font-bold'}`}>
                <BookOpen size={18} /> <span className="text-sm">Kelas Madin & Qur'an</span>
              </Link>
            </li>
            <li>
