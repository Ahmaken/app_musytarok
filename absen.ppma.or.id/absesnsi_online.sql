...
-- TABEL ABSENSI QURAN
-- =============================================
CREATE TABLE IF NOT EXISTS absensi_quran (
    id INT AUTO_INCREMENT PRIMARY KEY,
    tanggal DATE NOT NULL,
    jadwal_quran_id INT NOT NULL,
    murid_id INT NOT NULL,
    status ENUM('Hadir', 'Sakit', 'Izin', 'Alpa') NOT NULL,
    keterangan TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (jadwal_quran_id) REFERENCES jadwal_quran(id) ON DELETE CASCADE,
    FOREIGN KEY (murid_id) REFERENCES murid(murid_id) ON DELETE CASCADE,
    INDEX idx_tanggal (tanggal),
    INDEX idx_jadwal_quran (jadwal_quran_id),
    INDEX idx_murid (murid_id)
);

-- =============================================
-- TABEL ABSENSI KEGIATAN KAMAR
-- =============================================
CREATE TABLE IF NOT EXISTS `absensi_kegiatan` (
  `absensi_kegiatan_id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`absensi_kegiatan_id`),
  UNIQUE KEY `unique_absensi_kegiatan` (`kegiatan_id`,`murid_id`,`tanggal`),
  KEY `murid_id` (`murid_id`),
  CONSTRAINT `absensi_kegiatan_ibfk_1` FOREIGN KEY (`kegiatan_id`) REFERENCES `jadwal_kegiatan` (`kegiatan_id`) ON DELETE CASCADE,
  CONSTRAINT `absensi_kegiatan_ibfk_2` FOREIGN KEY (`murid_id`) REFERENCES `murid` (`murid_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- =============================================
-- UPDATE TABEL ABSENSI_KEGIATAN (sesuaikan dengan SQL yang ada)
-- =============================================
...
-- =============================================
-- TABEL ABSENSI KAMAR (jika belum ada)
-- =============================================
CREATE TABLE IF NOT EXISTS absensi_kamar (
    absensi_kamar_id INT AUTO_INCREMENT PRIMARY KEY,
    kamar_id INT NOT NULL,
    murid_id INT NOT NULL,
    tanggal DATE NOT NULL,
    waktu_masuk TIME NULL,
    waktu_keluar TIME NULL,
    status ENUM('Masuk', 'Keluar', 'Izin', 'Alpa') DEFAULT 'Alpa',
    keterangan TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (kamar_id) REFERENCES kamar(kamar_id) ON DELETE CASCADE,
    FOREIGN KEY (murid_id) REFERENCES murid(murid_id) ON DELETE CASCADE,
    UNIQUE KEY unique_kamar_murid_tanggal (kamar_id, murid_id, tanggal)
);
...
-- Tambahkan kolom status ke tabel guru jika perlu
ALTER TABLE guru ADD COLUMN IF NOT EXISTS status ENUM('aktif', 'nonaktif') DEFAULT 'aktif';
...
-- =============================================
-- BUAT INDEX UNTUK TABEL BARU
-- =============================================
CREATE INDEX IF NOT EXISTS idx_absensi_quran_tanggal ON absensi_quran(tanggal);
CREATE INDEX IF NOT EXISTS idx_absensi_quran_murid ON absensi_quran(murid_id);
CREATE INDEX IF NOT EXISTS idx_absensi_quran_jadwal ON absensi_quran(jadwal_quran_id);

CREATE INDEX IF NOT EXISTS idx_absensi_kegiatan_tanggal ON absensi_kegiatan(tanggal);
CREATE INDEX IF NOT EXISTS idx_absensi_kegiatan_murid ON absensi_kegiatan(murid_id);
CREATE INDEX IF NOT EXISTS idx_absensi_kegiatan_kegiatan ON absensi_kegiatan(kegiatan_id);

CREATE INDEX IF NOT EXISTS idx_absensi_kamar_tanggal ON absensi_kamar(tanggal);
CREATE INDEX IF NOT EXISTS idx_absensi_kamar_murid ON absensi_kamar(murid_id);
CREATE INDEX IF NOT EXISTS idx_absensi_kamar_kamar ON absensi_kamar(kamar_id);
...
-- Script untuk menambahkan tabel yang belum ada ke database existing
-- Jalankan script ini di database yang sudah ada

-- 1. Cek dan buat tabel absensi_kamar
CREATE TABLE IF NOT EXISTS absensi_kamar (
    absensi_kamar_id INT AUTO_INCREMENT PRIMARY KEY,
    kamar_id INT NOT NULL,
    murid_id INT NOT NULL,
...
-- 2. Tambahkan kolom kamar_id ke jadwal_kegiatan jika belum ada
ALTER TABLE jadwal_kegiatan 
ADD COLUMN IF NOT EXISTS kamar_id INT NULL AFTER nama_kegiatan;

-- Tambahkan foreign key jika belum ada
ALTER TABLE jadwal_kegiatan 
ADD CONSTRAINT fk_jadwal_kegiatan_kamar 
FOREIGN KEY (kamar_id) REFERENCES kamar(kamar_id) ON DELETE CASCADE;

-- 3. Tambahkan kolom timestamp ke absensi_kegiatan jika belum ada
ALTER TABLE absensi_kegiatan 
ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP;
