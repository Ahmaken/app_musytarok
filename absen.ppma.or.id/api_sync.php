...
function syncKelasMadin($conn) {
    echo "ðŸ”„ Sync data kelas_madin via API...\n";
...
function syncKamar($conn) {
    echo "ðŸ”„ Sync data kamar via API...\n";
...
function syncKelasQuran($conn) {
    echo "ðŸ”„ Sync data kelas_quran via API...\n";
...
function syncUsers($conn) {
    echo "ðŸ”„ Sync data users via API...\n";
...
function syncPengaturanNotifikasi($conn) {
    echo "ðŸ”„ Sync data pengaturan_notifikasi via API...\n";
...
function syncAbsensiGuru($conn) {
    echo "ðŸ”„ Sync data absensi_guru via API...\n";
...
function syncPengaturanAbsensiOtomatis($conn) {
    echo "ðŸ”„ Sync data pengaturan_absensi_otomatis via API...\n";
...
function syncJadwalMadin($conn) {
    echo "ðŸ”„ Sync data jadwal_madin via API...\n";
...
function syncJadwalQuran($conn) {
    echo "ðŸ”„ Sync data jadwal_quran via API...\n";
...
function syncJadwalKegiatan($conn) {
    echo "ðŸ”„ Sync data jadwal_kegiatan via API...\n";
...
function syncLoginAttempts($conn) {
    echo "ðŸ”„ Sync data login_attempts via API...\n";
...
// =============================================
// FUNGSI SINKRONISASI YANG BARU - DITAMBAHKAN
// =============================================

function syncAbsensiQuran($conn) {
    echo "🔄 Sync data absensi_quran via API...\n";
    $data = getDataFromPPMA('/absensi-quran');
    
    if (!$data || !is_array($data)) {
        return ['success' => false, 'message' => 'Gagal ambil data absensi_quran dari PPMA'];
    }
    
    return syncTableData($conn, 'absensi_quran', $data, 'id', [
        'tanggal', 'jadwal_quran_id', 'murid_id', 
        'status', 'keterangan', 'created_at', 'updated_at'
    ]);
}

function syncAbsensiKegiatan($conn) {
    echo "🔄 Sync data absensi_kegiatan via API...\n";
    $data = getDataFromPPMA('/absensi-kegiatan');
    
    if (!$data || !is_array($data)) {
        return ['success' => false, 'message' => 'Gagal ambil data absensi_kegiatan dari PPMA'];
    }
    
    return syncTableData($conn, 'absensi_kegiatan', $data, 'absensi_kegiatan_id', [
        'kegiatan_id', 'murid_id', 'tanggal', 
        'status', 'keterangan', 'waktu_dibuat'
    ]);
}

function syncAbsensiKamar($conn) {
    echo "🔄 Sync data absensi_kamar via API...\n";
    $data = getDataFromPPMA('/absensi-kamar');
    
    if (!$data || !is_array($data)) {
        return ['success' => false, 'message' => 'Gagal ambil data absensi_kamar dari PPMA'];
    }
    
    return syncTableData($conn, 'absensi_kamar', $data, 'absensi_kamar_id', [
        'kamar_id', 'murid_id', 'tanggal', 
...
// =============================================
// FUNGSI GET DATA YANG BARU - DITAMBAHKAN
// =============================================

function getAbsensiQuranData($conn, $limit = 100000) {
    $query = "
        SELECT 
            aq.id, aq.tanggal, 
            jq.mata_pelajaran, jq.hari,
            m.nama, m.nis,
            kq.nama_kelas as kelas_quran,
            aq.status, aq.keterangan,
            aq.created_at, aq.updated_at
        FROM absensi_quran aq
        JOIN jadwal_quran jq ON aq.jadwal_quran_id = jq.id
        JOIN murid m ON aq.murid_id = m.murid_id
        LEFT JOIN kelas_quran kq ON jq.kelas_quran_id = kq.id
        ORDER BY aq.tanggal DESC 
        LIMIT ?
    ";
    return getTableData($conn, $query, [$limit]);
}

function getAbsensiKegiatanData($conn, $limit = 100000) {
    $query = "
        SELECT 
            ak.absensi_kegiatan_id, ak.tanggal, 
            jk.nama_kegiatan, jk.hari,
            m.nama, m.nis,
            km.nama_kamar,
            ak.status, ak.keterangan,
            ak.waktu_dibuat as created_at
        FROM absensi_kegiatan ak
        JOIN jadwal_kegiatan jk ON ak.kegiatan_id = jk.kegiatan_id
        JOIN murid m ON ak.murid_id = m.murid_id
        LEFT JOIN kamar km ON jk.kamar_id = km.kamar_id
...
function getAbsensiKamarData($conn, $limit = 100000) {
    $query = "
        SELECT 
            ak.absensi_kamar_id, ak.tanggal,
            km.nama_kamar,
            m.nama, m.nis,
            ak.waktu_masuk, ak.waktu_keluar,
            ak.status, ak.keterangan,
            ak.created_at
        FROM absensi_kamar ak
        JOIN kamar km ON ak.kamar_id = km.kamar_id
        JOIN murid m ON ak.murid_id = m.murid_id
        ORDER BY ak.tanggal DESC, ak.waktu_masuk 
        LIMIT ?
    ";
    return getTableData($conn, $query, [$limit]);
}
...
try {
    ...
    switch($action) {
        case 'test':
            ...
            case 'sync_absensi_quran':
                $response = syncAbsensiQuran($conn);
                break;
                
            case 'sync_absensi_kegiatan':
                $response = syncAbsensiKegiatan($conn);
                break;
                
            case 'sync_absensi_kamar':
                $response = syncAbsensiKamar($conn);
                break;    
            ...
            case 'sync_all_with_new_tables':
                // Sinkronisasi SEMUA data termasuk tabel baru
                $results = [];
                $results[] = syncKelasMadin($conn);
                $results[] = syncKamar($conn);
                $results[] = syncKelasQuran($conn);
                ...
                $results[] = syncJadwalKegiatan($conn);
                $results[] = syncLoginAttempts($conn);
                $results[] = syncAbsensiQuran($conn);       // TABEL BARU
                $results[] = syncAbsensiKegiatan($conn);    // TABEL BARU
                $results[] = syncAbsensiKamar($conn);       // TABEL BARU
                
                $response = [
                    'success' => true,
                    'message' => 'Sinkronisasi SEMUA data (termasuk tabel baru) selesai',
                    'results' => $results,
                    'timestamp' => date('Y-m-d H:i:s')
                ];
                break;
        ...
        // GET DATA untuk tabel baru
        case 'get_absensi_quran':
            $limit = intval($_GET['limit'] ?? 100000);
            $data = getAbsensiQuranData($conn, $limit);
            $response = [
                'success' => true, 
                'data' => $data, 
                'total' => count($data),
                'message' => 'Data absensi quran berhasil diambil'
            ];
            break;
            
        case 'get_absensi_kegiatan':
            $limit = intval($_GET['limit'] ?? 100000);
            $data = getAbsensiKegiatanData($conn, $limit);
            $response = [
                'success' => true, 
                'data' => $data, 
                'total' => count($data),
                'message' => 'Data absensi kegiatan berhasil diambil'
            ];
            break;
            
        case 'get_absensi_kamar':
            $limit = intval($_GET['limit'] ?? 100000);
            $data = getAbsensiKamarData($conn, $limit);
            $response = [
                'success' => true, 
                'data' => $data, 
                'total' => count($data),
                'message' => 'Data absensi kamar berhasil diambil'
            ];
            break;
