...
if (isset($_SESSION['role']) && in_array($_SESSION['role'], ['wali_murid', 'wali_kelas'])) {
    $active_tab = 'dataMurid';
}

// Filter data untuk role guru - PERBAIKAN: cek session dulu
$guru_id = null;
...
$allowed_roles = ['admin', 'staff', 'guru', 'wali_murid'];
if (!isset($_SESSION['role']) || !in_array($_SESSION['role'], $allowed_roles)) {
    header("Location: ../index.php");
    exit();
}

// Role 'guru' diizinkan mengakses database.php untuk melihat data pribadi dan murid yang diajarnya.
...
if ($guru_id) {
    ...
    $stmt->bind_param("i", $_SESSION['murid_id']);
}
$stmt->execute();
$result = $stmt->get_result();
$murid_list = [];
...
<body>
    ...
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
            <?php if (!isset($_SESSION['role']) || !in_array($_SESSION['role'], ['wali_murid', 'wali_kelas'])): ?>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?= $active_tab == 'dataGuru&Pembina' ? 'active' : '' ?>" id="dataGuru&Pembina-tab" data-bs-toggle="tab" data-bs-target="#dataGuru&Pembina" type="button" role="tab">Data Guru & Pembina</button>
            </li>
            <?php endif; ?>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?= $active_tab == 'dataMurid' ? 'active' : '' ?>" id="dataMurid-tab" data-bs-toggle="tab" data-bs-target="#dataMurid" type="button" role="tab">Data Murid</button>
            </li>
        </ul>
