...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && (isset($_POST['simpan_absensi_kegiatan']) || isset($_POST['perbarui_absensi_kegiatan']))) {
    ...
    // PERBAIKAN: Pastikan tabel ada sebelum memproses
    if (!table_exists($conn, 'absensi_kegiatan')) {
        create_absensi_kegiatan_table($conn);
    }
...
function create_absensi_kegiatan_table($conn) {
    $sql = "CREATE TABLE IF NOT EXISTS absensi_kegiatan (
        absensi_kegiatan_id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        FOREIGN KEY (kegiatan_id) REFERENCES jadwal_kegiatan(kegiatan_id) ON DELETE CASCADE,
        FOREIGN KEY (murid_id) REFERENCES murid(murid_id) ON DELETE CASCADE,
        UNIQUE KEY unique_absensi_kegiatan (kegiatan_id, murid_id, tanggal)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci";
    
    if ($conn->query($sql) === TRUE) {
        error_log("✅ Tabel absensi_kegiatan berhasil dibuat/divalidasi");
        return true;
    } else {
        error_log("❌ Error membuat tabel absensi_kegiatan: " . $conn->error);
        return false;
    }
}
...
function cek_absensi_sudah_ada($conn, $type, $jadwal_id, $tanggal) {
    ...
    try {
        ...
        error_log("Error in cek_absensi_sudah_ada: " . $e->getMessage());
        return false;
    }
...
if ($kegiatan_selected) {
    ...
    if ($stmt) {
        ...
        if (table_exists($conn, 'absensi_kegiatan')) {
            $sql = "SELECT * FROM absensi_kegiatan WHERE kegiatan_id = ? AND tanggal = ?";
            $stmt = $conn->prepare($sql);
            if ($stmt) {
            ...
            error_log("⚠️ Tabel absensi_kegiatan tidak ditemukan, membuat tabel...");
            create_absensi_kegiatan_table($conn);
            // Setel variabel ke default
            $absensi_kegiatan_data = [];
            $is_absensi_kegiatan_sudah_ada = false;
        }
        if ($stmt) {
...
<body>
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="d-grid mt-3">
                                <?php if ($is_absensi_quran_sudah_ada): ?>
                                    <button type="submit" name="perbarui_absensi_quran" class="btn btn-warning" onclick="return confirmSimpanAbsensi('quran', this)">
                                ...
                                <?php else: ?>
                                    <button type="submit" name="simpan_absensi_quran" class="btn btn-secondary" onclick="return confirmSimpanAbsensi('quran', this)">
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="d-grid mt-3">
                                <?php if ($is_absensi_sudah_ada): ?>
                                    <button type="submit" name="perbarui_absensi" class="btn btn-warning" onclick="return confirmSimpanAbsensi('madin', this)">
                                        <i class="bi bi-arrow-clockwise me-1"></i> Perbarui Absensi
                                    </button>
                                    <small class="text-muted mt-1 d-block">Absensi untuk hari ini sudah pernah disimpan. Klik untuk memperbarui data.</small>
                                <?php else: ?>
                                    <button type="submit" name="simpan_absensi" class="btn btn-secondary" onclick="return confirmSimpanAbsensi('madin', this)">
                                        <i class="bi bi-check-circle me-1"></i> Simpan Absensi
                                    </button>
                                <?php endif; ?>
                            </div>
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="d-grid mt-3">
                                <?php if ($is_absensi_kegiatan_sudah_ada): ?>
                                    <button type="submit" name="perbarui_absensi_kegiatan" class="btn btn-warning" onclick="return confirmSimpanAbsensi('kegiatan', this)">
                                        <i class="bi bi-arrow-clockwise me-1"></i> Perbarui Absensi
                                    </button>
                                ...
                                <?php else: ?>
                                    <button type="submit" name="simpan_absensi_kegiatan" class="btn btn-secondary" onclick="return confirmSimpanAbsensi('kegiatan', this)">
                                        <i class="bi bi-check-circle me-1"></i> Simpan Absensi
                                    </button>
                                <?php endif; ?>
                            </div>
    ...
    <script>
        ...
        // Koordinat Pondok Pesantren Matholi'ul Anwar
        const PONDOK_LAT = -7.036195;
        const PONDOK_LNG = 112.350791;
        const MAX_RADIUS_METER = 50; // Toleransi 50 meter
        ...
        function confirmSimpanAbsensi(jenis, btnElement) {
            // Karena ini dipanggil dari onclick yang mungkin tidak melempar element, kita cari event-nya
            const e = window.event;
            if (e) e.preventDefault(); // Stop submit langsung

            const currentBtn = e ? e.target.closest('button') : null;
            if (!currentBtn) return false;

            const form = currentBtn.closest('form');
            const submitName = currentBtn.name;

            // Cek jumlah Alpa
            const countAlpa = $(form).find('select option:selected[value="Alpa"]').length;
            if (countAlpa > 0) {
                if (!confirm(`Ada ${countAlpa} siswa yang alpa. Lanjutkan?`)) {
                    return false;
                }
            }
            ...
            currentBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Melacak GPS...';
            currentBtn.disabled = true;

            if (!navigator.geolocation) {
                alert('Browser/HP Anda tidak mendukung pelacakan lokasi GPS.');
            ...
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    const guruLat = position.coords.latitude;
                    const guruLng = position.coords.longitude;
                    
                    const jarak = hitungJarakMeter(guruLat, guruLng, PONDOK_LAT, PONDOK_LNG);
                    
                    if (jarak <= MAX_RADIUS_METER) {
                        // Jarak aman, tambahkan input hidden agar PHP tahu tombol mana yang ditekan
                        ...
                        currentBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Menyimpan...';
                        form.submit();
                    } else {
                        // Di luar jangkauan
                        alert(`❌ AKSES DITOLAK!\n\nJarak Anda: ${jarak} meter dari titik pusat pondok.\nMaksimal toleransi jarak yang diizinkan adalah ${MAX_RADIUS_METER} meter.\n\nHarap lakukan absensi di lingkungan PP. Matholi'ul Anwar!`);
                        currentBtn.innerHTML = originalText;
                        currentBtn.disabled = false;
                    }
                },
                function(error) {
                    ...
                    if(error.code === 1) msg += "Izin GPS DITOLAK oleh Anda. Harap izinkan akses lokasi pada browser/sistem HP.";
                    else if(error.code === 2) msg += "Sinyal GPS tidak tersedia.";
                    else if(error.code === 3) msg += "Waktu pelacakan habis.";
                    alert(msg);
                    currentBtn.innerHTML = originalText;
                    currentBtn.disabled = false;
                },
