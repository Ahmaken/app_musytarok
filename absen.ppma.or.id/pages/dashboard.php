...
$allowed_roles = ['admin', 'staff', 'guru', 'wali_murid', 'wali_kelas'];
...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'guru') {
    ...
    $sql_pelanggaran .= " AND km.kelas_id = ?";
}
...
if ($stmt_pelanggaran) {
    if (isset($_SESSION['role']) && $_SESSION['role'] === 'guru') {
        ...
        $stmt_pelanggaran->bind_param("i", $_SESSION['kelas_id']);
    }
...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'guru') {
    ...
    $sql_perizinan .= " AND p.murid_id = ?";
} elseif (isset($_SESSION['role']) && $_SESSION['role'] === 'wali_kelas' && isset($_SESSION['kelas_id'])) {
    $sql_perizinan .= " AND km.kelas_id = ?";
}
...
if ($stmt_perizinan) {
    if (isset($_SESSION['role']) && $_SESSION['role'] === 'guru') {
        ...
        $stmt_perizinan->bind_param("i", $_SESSION['kelas_id']);
    }
...
<body>
    ...
    <div class="container mt-4">
        ...
        <?php if (in_array($_SESSION['role'], ['admin', 'staff', 'guru'])): ?>
        <div class="row mt-4">
    ...
    <script>
    // Konfigurasi Firebase Anda (Sesuai dengan kode asli dari Firebase Console)
    ...
    function requestNotificationPermission() {
        Notification.requestPermission().then((permission) => {
            if (permission === 'granted') {
                ...
                messaging.getToken({ vapidKey: "BKM9igOmbJUCvQj10NisCSJW3yzZwjpyQMnluluYbshJefCUfifjU4YwVBEU3U1kOhd-PWkKqPhZ3G8a84hoWr4" }).then((currentToken) => {
                    if (currentToken) {
                        console.log('FCM Token berhasil didapatkan:', currentToken);
                        // Simpan token ke database server kita
                        simpanTokenKeDatabase(currentToken);
                    } else {
                        console.log('Tidak bisa mendapatkan token, pastikan Vapid Key benar.');
                    }
                }).catch((err) => {
                    console.log('Gagal mendapatkan token FCM ', err);
                });
            } else {
    ...
    // Fungsi menyimpan token via AJAX
    function simpanTokenKeDatabase(token) {
        fetch('../simpan_fcm_token.php', {
            method: 'POST',
            headers: {
        ...
        .then(response => response.json())
        .then(data => {
            console.log('Respons Server:', data.message);
        })
        .catch(err => {
    ...
    // Tangani pesan saat aplikasi sedang dibuka (Foreground)
    messaging.onMessage((payload) => {
        console.log('Pesan FCM masuk (Foreground): ', payload);
        // Tampilkan peringatan sederhana menggunakan UI/alert
        if(payload.notification) {
            // Bisa diganti menggunakan Toast/Modal yang lebih cantik
            alert("ðŸ”” NOTIFIKASI BARU: " + payload.notification.title + "\n" + payload.notification.body);
        }
    });

    // Jalankan permintaan izin beberapa detik setelah halaman dimuat (agar tidak mengganggu)
    setTimeout(() => {
